...
team_t team = {
    ...
    "Julia Bristow",      /* First member full name */
    "julia_bristow@gwu.edu",  /* First member email address */
...
{
        int ii, jj;
	int i2 = (i-1 > 0 ? i-1:0), i3 = (i+1 < dim-1 ? i+1:dim-1);
	int j2 = (j-1 > 0 ? j-1:0), j3 = (j+1 < dim-1 ? j+1:dim-1);
        pixel_sum sum;
        pixel current_pixel;
